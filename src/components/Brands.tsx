    ...
    return (
        <div className="w-full space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {brandsList.map((item) => (
                    <Link to={`/category/${category.id}/${item.marca.toLowerCase()}`} className="space-y-1">
                        <img src={item.img_header} className="rounded-xl object-cover transition-shadow duration-300 hover:shadow-lg cursor-pointer" />
